...
from lib.dataset import load_predefined_train_test_idx
...
def write_board(writer, epoch, tloss, tacc, acc, lr):
...
for fold, (train_idxs, test_idxs) in enumerate(kfold.split(kfold_x, kfold_y)):
    ...
    for epoch in range(start_epoch, limit_epoch+1):
        ...
        write_board(writer, epoch, train_loss, train_acc, val_accuracy, current_lr)
