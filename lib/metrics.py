...
# TODO: caclulate metric in pytorch
# need a different way to store graph variable in pytorch?
...
def confusion_matrix(tp, fp, fn, tn, num_labels=1, scope='confusion_matrix'):
    cm = torch.shape(torch.stack([tp, fp, fn, tn], 0), [num_labels, 2, 2])
    return cm.type(torch.IntTensor)
