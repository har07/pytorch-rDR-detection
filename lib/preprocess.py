    ...
    for i, image_path in enumerate(image_paths):
        ...
        try:
            ...
            if processed is None:
                ...
                image_png_filename = "{0}.png".format(os.path.splitext(
                                        os.path.basename(image_filename))[0])
                output_path = os.path.join(save_path, image_png_filename)

                # Save the image.
                cv2.imwrite(output_path, processed,
                            [int(cv2.IMWRITE_PNG_COMPRESSION), 100])
...
def resize(images_paths, size=299):
    ...
    for image_path in images_paths:
        ...
        # Save the image.
        cv2.imwrite(image_path, image, [int(cv2.IMWRITE_PNG_COMPRESSION), 100])
...
def rescale_min_1_to_1(image):
    ...
    image = torch.mul(image, 1. / 127.5)
    # Rescale to [-1, 1].
    return torch.sub(image, 1.0)
