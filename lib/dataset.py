...
def load_split_train_test(datadir, bs=50, valid_bs=57, valid_size=.2):
    ...
    trainloader = torch.utils.data.DataLoader(train_data, sampler=train_sampler, batch_size=bs, shuffle=True)
    testloader = torch.utils.data.DataLoader(test_data, sampler=test_sampler, batch_size=valid_bs, shuffle=True)
